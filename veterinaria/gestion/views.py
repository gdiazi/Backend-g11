...
from .serializers import RegistroUsuarioSerializer, MascotasSerializer, LeerUsuarioSerializer
from .models import Usuario, Mascota
from rest_framework.response import Response
from rest_framework import status
# https://www.django-rest-framework.org/api-guide/permissions/
from rest_framework.permissions import IsAuthenticated
from .permisions import SoloClientes
...
class DevolverUsuario(APIView):
    
    def get(self, request:Request):
            usuarios = Usuario.objects.all()
            serializador = LeerUsuarioSerializer(usuarios, many=True)

            return Response(data = {
                'content': serializador.data
            }, status=status.HTTP_200_OK)    
        
class MascotasView(generics.ListCreateAPIView):
